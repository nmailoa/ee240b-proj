...
\hline
M11/12 & 88.56 & 200 & 0.15 & 1.8 & 23.8 & 17.5 \\ %Pm77
\hline
...
The device sizings for the second stage is scaled by a factor. The differential input of the OTA is sized by a factor of 0.22 while the output branch of the OTA is sized by a factor of 0.3. Every other sizing including the gain boosters are the same and the two stages share the same bias network.
...
Mainly, we would have used extra time if we had it for the second stage. The second stage needs to drive a feedback capacitor that is 15 times smaller than the first stage. This means that the current should be a tenth or so of the first stage. We just scaled the current in the diff pair down by 5 times. The problem is that in order to maintain the same gain, current balance in the two branches, and phase margin, the entire OTA would need to be re-biased and re-sized. We chose the faster method of making up the difference with extra load cap.
